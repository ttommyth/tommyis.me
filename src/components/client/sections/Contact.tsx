...
const followUpMessage = [
  'Could you please provide me with your email address? This will help me get in touch with you.',
  "Could you please tell me a bit about the content you're interested in? Are you looking for specific projects or information related to my skills?",
  'Click the button below to send the message to me!',
];
...
const validationSchema = yup.object({
  ...
  email: yup
    .string()
    .max(1024, 'Max length reached')
    .email('Email Invalid')
    .required('Required'),
  content: yup.string().max(2048, 'Max length reached').required('Required'),
});

const MessageLine: FC<PropsWithChildren<{ from: 'me' | 'user' }>> = ({
  from,
  children,
}) => {
  return (
    <div className="flex gap-4">
      <div
        className="w-8 h-8 px-4 bg-primary-500 data-[from='user']:bg-secondary-500 flex justify-center items-center text-base-100 font-black"
        data-from={from}
      >
        {from == 'me' ? 'T' : 'U'}
      </div>
      <div className="grow leading-8">{children}</div>
    </div>
  );
};
      ...
      <div className="min-h-[100dvh] min-w-[320px] flex justify-center items-center px-0 pt-appbar sm:pt-4 sm:py-4 h-[100dvh] sm:h-auto ">
        <div className=" flex justify-stretch items-stretch w-full bg-base-300 dark:bg-base-900 border-default border-2  rounded-md flex-col sm:flex-row h-full sm:h-auto max-h-[100dvh] sm:max-h-[80dvh]">
          <div className="flex flex-row sm:flex-col h-auto sm:h-full [&>*]:p-2 [&>*]:rounded-md [&>*]:flex [&>*]:gap-4 [&>*]:items-center gap-2 p-2 text-sm pb-2 sm:pb-48 whitespace-nowrap">
            <span className="bg-base-200 dark:bg-base-800">
              <ChatBubbleLeftEllipsisIcon className="w-icon h-icon" />
              <span className="hidden sm:inline-block">Website</span>
            </span>
            <Link
              href={'https://www.google.com/maps/place/%E9%A6%99%E6%B8%AF'}
              target="_blank"
              className="dui-tooltip dui-tooltip-bottom sm:dui-tooltip-top"
              data-tip="Find me in"
            >
              <MapPinIcon className="w-icon h-icon " />
              <span className="hidden sm:inline-block ">Hong Kong</span>
            </Link>
            <button
              className="dui-tooltip dui-tooltip-bottom sm:dui-tooltip-top"
              data-tip="Click to copy"
              type="button"
              onClick={(ev) => navigator.clipboard.writeText(email)}
            >
              <EnvelopeIcon className="w-icon h-icon" />
              <span className="hidden  sm:flex sm:items-center gap-2">
                Email
                <Link
                  href={`mailto:${email}?subject=Hi+Tommy`}
            ...
            <Link href={linkedinLink} className="" target="_blank">
              <LinkedinLogo className="w-icon h-icon " />
          ...
          <div className="bg-default grow flex flex-col overflow-y-hidden relative">
            <div className="flex flex-col grow bg-dotted [&>div]:pl-4 [&>div]:p-8 [&>div:nth-child(odd)]:bg-base-300/50 [&>div:nth-child(odd)]:dark:bg-base-900/50 py-2 overflow-y-auto">
              <MessageLine from="me">
                Hello there! This is an contact me form, utilizing React Hook
                Form and Yup for seamless functionality, bolstered by reCAPTCHA
                v3 to ensure enhanced security. Please enter your name in the
                input box below! 🧑🏻‍💻
              </MessageLine>
                  ...
                  {stepArr.indexOf(step) > stepArr.indexOf(it) ? (
                    <>
                      ...
                      {followUpMessage[stepArr.indexOf(it)] ? (
                        <MessageLine from="me">
                          <TypeWriterSpan
                            showCursor={stepArr.indexOf(step) == idx + 1}
                            onEvent={(ref, ev) => ref.scrollIntoView()}
                          >
                            {followUpMessage[stepArr.indexOf(it)]}
                          </TypeWriterSpan>
                        </MessageLine>
              ...
              {step == 'submitted' ? (
                <MessageLine from="me">Thanks for your message!</MessageLine>
              ) : (
                <></>
              )}
            </div>
            ...
            >
              <span className="relative group">
                {step == 'submitted' ? (
                  ...
                  <button
                    type="submit"
                    className="flex items-center gap-4 px-4 py-2 border-default-invert border-2 bg-base-200 dark:bg-base-900 mx-auto rounded-xl"
                  >
                  ...
                  <>
                    ...
                    <button
                      ...
                      disabled={
                        !!formState.errors[step] || !formState.dirtyFields[step]
                      }
                    >
                    ...
                    <span className="text-sm absolute top-full pt-1 left-2 text-red-500">
